...
\end{DoxyParagraph}
\hypertarget{md_pages_tutorials_tutorial2_autotoc_md40}{}\doxysection{What is a default Galerkin problem?}\label{md_pages_tutorials_tutorial2_autotoc_md40}
...
\end{DoxyParagraph}
where $\hat{y}$ is the reduced state, also called generalized coordinates, $y$ is the full-\/order model (F\+OM) state, $y_{ref}$ is a reference F\+OM state, $\phi$ is the orthonormal basis, and $f(...)$ is the F\+OM velocity. Schematically, this system corresponds to the figure below. \hypertarget{md_pages_tutorials_tutorial2_autotoc_md41}{}\doxysection{How to create a default Galerkin problem?}\label{md_pages_tutorials_tutorial2_autotoc_md41}
...
The optional argument allows one to pass an \char`\"{}observer\char`\"{} object whose purpose is to monitor the evolution of the reduced state. The observer is called back by pressio4py during the time integration at every time step. This can be useful to, e.\+g., save the generalized coordinates, or usign them to perfom some other operation.\hypertarget{md_pages_tutorials_tutorial2_autotoc_md43}{}\doxysection{Putting all steps together}\label{md_pages_tutorials_tutorial2_autotoc_md43}
